...
def get_returns(experiment):
    ...
    """
    asset_list = experiment.get_asset_list()
    returns_asset_list = [a for a in asset_list if a['fileName'] == 'eval_returns']
    if len(returns_asset_list) > 0:
        returns_asset = returns_asset_list[0]
        returns_json_str = experiment.get_asset(returns_asset['assetId'])
        returns = json.loads(returns_json_str)
        steps = np.arange(len(returns)) * 1e3
    else:
        returns_asset_list = experiment.get_metrics('cumulative_reward')
        returns = [float(a['metricValue']) for a in returns_asset_list]
        steps = [float(a['step']) for a in returns_asset_list]
    return np.array(returns), np.array(steps) / 1e3
...
def plot_density_animation(visual_dict, save_folder):
    ...
    # Plot formatting (done manually for this experiment)
    ax[0].set_xlim(-6,6)
    ax[1].set_xlim(0, 4)
    ax[2].set_xlim(-1, 1)
    ax[3].set_xlim(0, 1.5)
    ...
    anim = camera.animate()
    print(save_folder + 'density_animation.gif')
